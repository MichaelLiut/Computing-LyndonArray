...
void datasets(Lstring& x) {
  ...
  if (init) {     //empty all datasets
    ...
    for(d=10; d<=LEN; d=10*d) {
      sprintf(outfile,"extreme_trla2_%lu.dat",d);
      FILE* fp=fopen(outfile,"w");
      if (5*d <= LEN) {
        sprintf(outfile,"extreme_trla2_%lu.dat",5*d);
        FILE* fp=fopen(outfile,"w");
      }
    }
  }
  ...
  for(d=10; d <= x.len; d=10*d) {
    sprintf(outfile,"extreme_trla2_%d.dat",d);
    printf("appending \"%s\"\n",outfile);
    fp=fopen(outfile,"a");
    if (fp == 0) {
    ...
    d1=5*d;
    if (d1<=x.len) {
      sprintf(outfile,"extreme_trla2_%lu.dat",d1);
      printf("appending \"%s\"\n",outfile);
      fp=fopen(outfile,"a");
        if (fp == 0) {
        printf("can't open output file \"%s\"\n",outfile);
